    ...
    FOR t_row in SELECT paging_token, effect_id, asset_type, asset_code, asset_issuer, created_at FROM effects ORDER BY effect_id LOOP
        ...
        VALUES (t_row.paging_token, t_row.asset_code, t_row.asset_issuer, t_row.asset_type, t_row.created_at,
            (SELECT COALESCE(SUM(amount), 0) FROM effects
                ...
                AND account = t_row.asset_issuer
                AND asset_code = t_row.asset_code
                AND asset_issuer = t_row.asset_issuer
                AND effect_id <= t_row.effect_id),
            (SELECT COALESCE(SUM(amount), 0) FROM effects
                WHERE type = 'account_debited'
                AND account != t_row.asset_issuer
                AND asset_code = t_row.asset_code
            ...
            (SELECT COUNT(DISTINCT account) FROM effects
                WHERE account != t_row.asset_issuer
                AND type = 'trustline_created'
                AND asset_code = t_row.asset_code
                AND asset_issuer = t_row.asset_issuer
                AND  effect_id <= t_row.effect_id),
            (SELECT COUNT(DISTINCT account) FROM effects
                WHERE account != t_row.asset_issuer
                AND type = 'account_debited'
                AND asset_code = t_row.asset_code
                AND asset_issuer = t_row.asset_issuer
                AND  effect_id <= t_row.effect_id),
